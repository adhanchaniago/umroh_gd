...
if(isset($_POST['btnSimpan'])){
  ...
  if (count($pesanError)>=1 ){
    ...
    $mySql    = "UPDATE `los1n` SET 
            `time`            = '$Time', 
            `event`           = '$Event',
            `title`           = '$Title', 
            `description`     = '$Description',
            `URL`             = '$URL'
            WHERE id_los1n    ='".$_POST['txtKode']."'";


    $myQry=mysql_query($mySql, $Link) or die ("Gagal query".mysql_error());
    if($myQry){
      echo "<meta http-equiv='refresh' content='0; url=?page=LOS1N'>";
    }
    exit;
  } 
...
$mySql  = "SELECT * FROM los1n WHERE id_los1n='$Kode'";
