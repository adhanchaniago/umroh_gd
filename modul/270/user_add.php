...
# VALIDASI USERNAME LOGIN (username), jika sudah ada akan ditolak
  $cekSql="SELECT * FROM user WHERE Email='$Username'";
  ...
  if (count($pesanError)>=1 ){
    ...
    $mySql    = "INSERT INTO user (Travelname, Email, Password, RoleID,
                     Status, Salutation, FirstName, LastName, Phone, Gender, Dummy, CreatedDate, ExpirationDate)
            VALUES ('$Travelname', '$Username', MD5('$Password'),   '$RoleID',
                '$Status', '$Salutation', '$FirstName', '$LastName', '$Phone', '$Gender', '$Dummy', NOW(), '$ExpirationDate')";
...
$dataTravelname   = isset($_POST['Travelname']) ? $_POST['Travelname'] : '';
$dataUsername = isset($_POST['Username']) ? $_POST['Username'] : '';
$dataPassword = isset($_POST['Password']) ? $_POST['Password'] : '';
$dataRoleID = isset($_POST['RoleID']) ? $_POST['RoleID'] : '';
$dataStatus  = isset($_POST['Status']) ? $_POST['Status'] : '';
$dataSalution    = isset($_POST['Salutation']) ? $_POST['Salutation'] : '';
$dataFirstName    = isset($_POST['FirstName']) ? $_POST['FirstName'] : '';
$dataLastName    = isset($_POST['LastName']) ? $_POST['LastName'] : '';
$dataBirthDate    = isset($_POST['BirthDate']) ? $_POST['BirthDate'] : '';
$dataPhone    = isset($_POST['Phone']) ? $_POST['Phone'] : '';
$dataGender    = isset($_POST['Gender']) ? $_POST['Gender'] : '';
$dataDummy    = isset($_POST['Dummy']) ? $_POST['Dummy'] : '0';
$dataExpirationDate    = isset($_POST['ExpirationDate']) ? $_POST['ExpirationDate'] : '';
?>
                    ...
                    <tr>
                      ...
                      <td >
                        <select name="Travelname" required class="form">
                                  <option value="">....</option>
                                  <?php
                        	  $bacaSql = "SELECT * FROM travel ORDER BY travel_id";
                        	  $bacaQry = mysql_query($bacaSql, $Link) or die ("Gagal Query".mysql_error());
                        	  while ($bacaData = mysql_fetch_array($bacaQry)) {
                        		if ($bacaData['travel_id'] == $dataTravelname) {
                        			$cek = " selected";
                        		} else { $cek=""; }

                        		echo "<option value='$bacaData[travel_name]' $cek>  $bacaData[travel_name]</option>";
                        	  }
                        	  ?>
                                </select>
