...
# VALIDASI USERNAME LOGIN (username), jika sudah ada akan ditolak
  $cekSql="SELECT * FROM equipment WHERE equipment_name='$EquipmentName'";
  $cekQry=mysql_query($cekSql, $Link) or die ("Eror Query".mysql_error()); 
  if(mysql_num_rows($cekQry)>=1){
    $pesanError[] = "Equipment <b> $EquipmentName </b> sudah ada, ganti dengan yang lain";
  }
  ...
  if (count($pesanError)>=1 ){
    ...
    $mySql    = "INSERT INTO equipment (equipment_name, stock)
            VALUES ('$EquipmentName', '$Stock')";
...
$dataEquipmentName   = isset($_POST['EquipmentName']) ? $_POST['EquipmentName'] : '';
$dataStock = isset($_POST['Stock']) ? $_POST['Stock'] : '';
