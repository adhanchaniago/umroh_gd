...
$pageSql = "SELECT * FROM waiting WHERE packages_program='Super Saver' ";
...
if(isset($_POST['btnCari'])){
  if($_POST) {
    ...
    $mySql = "SELECT * FROM waiting WHERE  first_name LIKE '%$txtKataKunci%'
          ORDER BY waiting_id ASC LIMIT $hal, $row";
  }
}
else {
  $mySql = "SELECT * FROM waiting WHERE packages_program='Super Saver' ORDER by waiting_id ASC LIMIT $hal, $row";
}
                        ...
                        <article class="widget">
                          <form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="form1" target="_self" id="form1"  class="form-horizontal" role="form">
                                           <select name="txtKataKunci" bgcolor="blue" class="btn blue ">
                                              <option value="">--Select Jamaah--</option>
                                             <?php
                  $bacaSql = "SELECT * FROM waiting WHERE packages_program='Super Saver'";
                  $bacaQry = mysql_query($bacaSql, $Link) or die ("Gagal Query".mysql_error());
                  while ($bacaData = mysql_fetch_array($bacaQry)) {
                  if ($bacaData['waiting_id'] == $dataType) {
                    $cek = " selected";
                  } else { $cek=""; }

                  echo "<option value='$bacaData[first_name]' $cek>[ $bacaData[waiting_id] ]  $bacaData[first_name]  $bacaData[last_name]  $bacaData[surname]</option>";
                  }
                  ?>
                                              </select>
...
<?php
  ...
  ?>
        ...
        <td>
       <a href="?page=input_waiting_booking&NomorUmroh=<?php echo $myData['waiting_id']; ?>" target="_self" alt="Edit Data" id="kuota"  class='btn btn-xs btn-success ace-icon fa fa-pencil bigger-120'> Booking</a>&nbsp;
         <a href="?page=WaitingDeleteSuper&Kode=<?php echo $Kode; ?>" target="_self" alt="Delete Data" onclick="return confirm('ARE YOU SURE REMOVING THIS DATA  ... ?')" target="_self" alt="Delete Data" class='btn btn-xs btn-danger ace-icon fa fa-trash-o bigger-120'> Delete</a>
     </td>
         </tr>
        <?php } ?>
         </tbody>
...
?>
        ...
        <?php
        echo "
            <a href='?page=DataWaiting_super&hal=$prev'>
        ...
        <li class="">
            ...
            for ($h = 1; $h <= $max; $h++) {
                $list[$h] = $row * $h - $row;
                echo " <a href='?page=DataWaiting_super&hal=$list[$h]'>$h</a> ";
            }
            ?>
        </li>
        <li class="next">
                <?php
        echo "
            <a href='?page=DataWaiting_super&hal=$next'>
