...
$pageSql = "SELECT jamaah_daftar.*, track_jamaah.packages_program, track_jamaah.kd_umroh, track_jamaah.depart, track_jamaah.mahrom, track_jamaah.mahrom_status
 FROM jamaah_daftar
...
if(isset($_POST['btnCari'])){
  if($_POST) {
    ...
    $mySql = "SELECT DISTINCT  jamaah_daftar.*,track_jamaah.packages_program, track_jamaah.kd_umroh, track_jamaah.depart, track_jamaah.mahrom, track_jamaah.mahrom_status
     FROM jamaah_daftar
     LEFT JOIN track_jamaah on jamaah_daftar.nomor_id=track_jamaah.nomor_id
    WHERE  depart LIKE '%$txtKataKunci%' and packages_program='Rahmah'
          ORDER BY mahrom ASC LIMIT $hal, $row";
  }
}
else {
  $mySql = "SELECT jamaah_daftar.*,  track_jamaah.packages_program, track_jamaah.kd_umroh, track_jamaah.depart, track_jamaah.mahrom, track_jamaah.mahrom_status
   FROM jamaah_daftar
   LEFT JOIN track_jamaah on jamaah_daftar.nomor_id=track_jamaah.nomor_id
  WHERE packages_program='Rahmah' ORDER BY mahrom ASC LIMIT $hal, $row";
}
...
<section class="content">
    ...
    <div class="row">
            ...
            <form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="form1" target="_self" id="form1"  class="form-horizontal" role="form">
                            <select name="txtKataKunci" bgcolor="blue" class="btn blue ">
                               ...
                               <?php
    $bacaSql = "SELECT DISTINCT  jamaah_daftar.*,  track_jamaah.packages_program, track_jamaah.kd_umroh, track_jamaah.depart, track_jamaah.mahrom, track_jamaah.mahrom_status
     FROM jamaah_daftar
     LEFT JOIN track_jamaah on jamaah_daftar.nomor_id=track_jamaah.nomor_id
    WHERE packages_program='Rahmah'";
    $bacaQry = mysql_query($bacaSql, $Link) or die ("Gagal Query".mysql_error());
    while ($bacaData = mysql_fetch_array($bacaQry)) {
    if ($bacaData['mahrom'] == $dataKataKunci) {
      $cek = " selected";
    } else { $cek=""; }
    ...
    <article class="widget">
        ...
        <tr>
            ...
            <th style="text-align: center; width: 10%">Depart</th>
            ...
            <th style="text-align: center;">Mahrom Name</th>
...
<?php
  ...
  while ($myData = mysql_fetch_array($myQry)) {
	...
	$status = $myData['mahrom_status'];

   $DE == NEED_MAHROM;
    if ($status == $DE) {
  		$view2 = "<span class='btn inverse green'>ok</span>";
  		}
  ...
  ?>
        ...
        <td><?php echo $myData['mahrom']; ?></td>
         <td><?php echo $myData['mahrom_status']; ?> <!--<?php echo $view2; ?>--></td>
        <td> <a href="?page=DataEditMahromRahmah&Kode=<?php echo $Kode; ?>" target="_self" alt="Edit Data" class='btn btn-xs btn-inverse ace-icon fa fa-pencil bigger-120'> Add Mahrom</a>&nbsp;
     </td>
         </tr>
        <?php } ?>
...
<li class="" >
<strong style="font-weight: bold;">* All data from packages Rahmah Total Records :</strong>
                                <?php echo $jml; ?>
</ul>
...
?>
        ...
        <?php
        echo "
            <a href='?page=DataMahromRahmah&hal=$prev'>
        ...
        <li class="">
            ...
            for ($h = 1; $h <= $max; $h++) {
                $list[$h] = $row * $h - $row;
                echo " <a href='?page=DataMahromRahmah&hal=$list[$h]'>$h</a> ";
            }
            ?>
        </li>
        <li class="next">
                <?php
        echo "
            <a href='?page=DataMahromRahmah&hal=$next'>
