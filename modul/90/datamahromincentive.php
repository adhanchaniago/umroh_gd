...
if(isset($_POST['btnCari'])){
  ...
  $mySql = "SELECT jamaah_daftar.*,  track_jamaah.packages_program, track_jamaah.kd_umroh, track_jamaah.depart, track_jamaah.mahrom, track_jamaah.mahrom_status
   FROM jamaah_daftar
   LEFT JOIN track_jamaah on jamaah_daftar.nomor_id=track_jamaah.nomor_id
  WHERE packages_program='Incentive' ORDER BY mahrom ASC LIMIT $hal, $row";
}
...
<?php
  ...
  ?>
        ...
        <td> <a href="?page=DataEditMahromIncentive&Kode=<?php echo $Kode; ?>" target="_self" alt="Edit Data" class='btn btn-xs btn-inverse ace-icon fa fa-pencil bigger-120'> Add Mahrom</a>&nbsp;
     </td>
         </tr>
        <?php } ?>
...
<li class="" >
<strong style="font-weight: bold;">* All data from packages Incentive Total Records :</strong>
                                <?php echo $jml; ?>
</ul>
...
?>
        ...
        <?php
        echo "
            <a href='?page=DataMahromIncentive&hal=$prev'>
        ...
        <li class="">
            ...
            for ($h = 1; $h <= $max; $h++) {
                $list[$h] = $row * $h - $row;
                echo " <a href='?page=DataMahromIncentive&hal=$list[$h]'>$h</a> ";
            }
            ?>
        </li>
        <li class="next">
                <?php
        echo "
            <a href='?page=DataMahromIncentive&hal=$next'>
