...
if(isset($_POST['btnSimpan'])){
  ...
  if (count($pesanError)>=1 ){
    ...
    $mySql  = "UPDATE track_jamaah SET   room = '$Room', nomor_room = '$Nomor_Room'
          WHERE nomor_id ='".$_POST['txtKode']."'";
    $myQry  = mysql_query($mySql, $Link) or die ("Gagal query".mysql_error());
    if($myQry){
      echo "<meta http-equiv='refresh' content='0; url=?page=DataRoomlistBerkah'>";
    }
    exit;
  }
...
$mySql  = "SELECT * FROM track_jamaah WHERE nomor_id='$Kode'";
