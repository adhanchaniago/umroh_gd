...
<?php
session_start();
if($_SESSION['FirstName'] == 'DataJamaahIncentive') {header('Location: ?page=DataJamaahIncentive');} ;
...
WHERE packages_program='Incentive' and travel='$Travel' ";
...
if(isset($_POST['btnCari'])){
  if($_POST) {
    ...
    FROM jamaah_daftar
    LEFT JOIN track_jamaah on jamaah_daftar.nomor_id=track_jamaah.nomor_id
    WHERE  depart LIKE '%$txtKataKunci%' and packages_program='Incentive'  and travel='$Travel'
  ...
  FROM jamaah_daftar
  LEFT JOIN track_jamaah on jamaah_daftar.nomor_id=track_jamaah.nomor_id
  WHERE packages_program='Incentive'  and travel='$Travel' ORDER BY nomor_id ASC LIMIT $hal, $row";
}
            ...
            <header class="main-header">
                <div class="main-header__nav">
                    <h1 class="main-header__title">
                        ...
                        <span>Data Jamaah Incentive </span>
                    </h1>
...
<?php
  ...
  ?>
        ...
        <a href="?page=edit_jamaah&Kode=<?php echo $Kode; ?>" target="_self" alt="Edit Data"  class='btn btn-xs btn-success ace-icon fa fa-pencil bigger-120' title="Edit"><i class="pe-7s-edit" ></i></a>&nbsp;
           <a href="?page=Document_Personal_Incentive&NomorID=<?php echo $myData['nomor_id']; ?> " target="_self" alt="Detail Data" class='btn btn-xs btn-info ace-icon fa fa-eye bigger-120' title="Document"><i class="pe-7s-folder" ></i></a>&nbsp;
...
?>
        ...
        <?php
        echo "
            <a href='?page=DataJamaahIncentive&hal=$prev'>
        ...
        <li class="">
            ...
            for ($h = 1; $h <= $max; $h++) {
                $list[$h] = $row * $h - $row;
                echo " <a href='?page=DataJamaahIncentive&hal=$list[$h]'>$h</a> ";
            }
            ?>
        </li>
        <li class="next">
                <?php
        echo "
            <a href='?page=DataJamaahIncentive&hal=$next'>
