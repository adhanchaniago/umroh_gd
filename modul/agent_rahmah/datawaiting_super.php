...
<?php
session_start();
if($_SESSION['FirstName'] == 'DataWaiting_rahmah') {header('Location: ?page=DataWaiting_rahmah');} ;
...
$pageSql = "SELECT * FROM jamaah WHERE packages_program='rahmah'  and travel='$Travel' ";
...
if(isset($_POST['btnCari'])){
  if($_POST) {
    ...
    $mySql = "SELECT * FROM jamaah WHERE  first_name LIKE '%$txtKataKunci%' and packages_program='rahmah'  and travel='$Travel'
          ORDER BY nomor_id ASC LIMIT $hal, $row";
  }
}
else {
  $mySql = "SELECT * FROM jamaah WHERE packages_program='rahmah'  and travel='$Travel'  ORDER by nomor_id ASC LIMIT $hal, $row";
}
            ...
            <header class="main-header">
                <div class="main-header__nav">
                    <h1 class="main-header__title">
                        ...
                        <span>Jamaah rahmah Unpaid</span>
                    </h1>
                        ...
                        <article class="widget">
                          <form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="form1" target="_self" id="form1"  class="form-horizontal" role="form">
                                           <select name="txtKataKunci" bgcolor="blue" class="btn blue ">
                                             ...
                                             <?php
                  $bacaSql = "SELECT * FROM jamaah WHERE packages_program='rahmah'  and travel='$Travel'";
...
<?php
  ...
  ?>
      ...
      <a href="90/jamaah_print.php?NomorJamaah=<?php echo $myData['nomor_id']; ?>" target="_self" alt="Detail Data" class='btn btn-xs btn-info  ' title="Detail"> Detail</a>&nbsp;
       <a href="?page=Input_Agent_Booking_Rahmah&NomorJamaah=<?php echo $myData['nomor_id']; ?>" target="_self" alt="Edit Data" onclick="return confirm('ARE YOU SURE BOOKING THIS DATA  ... ? Data akan hilang dan masuk ke jamaah register klik Ok untuk melanjutkan ke pembayaran , klik cancel untuk kembali ')" id="kuota"  class='btn btn-xs btn-success '> Register</a>&nbsp;
         <!-- <a href="?page=WaitingDeleterahmah&Kode=<?php echo $Kode; ?>" target="_self" alt="Delete Data" onclick="return confirm('ARE YOU SURE REMOVING THIS DATA  ... ?')" target="_self" alt="Delete Data" class='btn btn-xs btn-danger ace-icon fa fa-trash-o bigger-120'> Delete</a> -->
     </td>
         </tr>
        <?php } ?>
...
?>
        ...
        <?php
        echo "
            <a href='?page=DataWaiting_rahmah&hal=$prev'>
        ...
        <li class="">
            ...
            for ($h = 1; $h <= $max; $h++) {
                $list[$h] = $row * $h - $row;
                echo " <a href='?page=DataWaiting_rahmah&hal=$list[$h]'>$h</a> ";
            }
            ?>
        </li>
        <li class="next">
                <?php
        echo "
            <a href='?page=DataWaiting_rahmah&hal=$next'>
