...
<?php
session_start();
if($_SESSION['FirstName'] == 'Rahmah-Agent') {header('Location: ?page=Rahmah-Agent');} ;
...
$pageSql = "SELECT * FROM paket_umroh WHERE nama_paket='Rahmah' ";
...
if(isset($_POST['btnCari'])){
  if($_POST) {
    ...
    $mySql = "SELECT * FROM paket_umroh WHERE  depart_umroh LIKE '%$txtKataKunci%' and nama_paket='Rahmah'
          ORDER BY kd_umroh ASC LIMIT $hal, $row";
  }
}
else {
  $mySql = "SELECT * FROM paket_umroh WHERE nama_paket='Rahmah' ORDER BY kd_umroh ASC LIMIT $hal, $row";
}
...
<!-- class="my_select_box" data-placeholder="Select Your Options" -->
  <select  name="txtKataKunci" bgcolor="blue" class="btn blue ">
                                <option value="">--Pilih Tanggal Keberangkatan--</option>
                               <?php
    $bacaSql = "SELECT distinct * FROM paket_umroh WHERE nama_paket='Rahmah'";
...
<?php
  ...
  ?>
        ...
        <td style="text-align: center"><?php echo $view; ?> <?php echo $viewa; ?> <?php echo $selisih; ?> Hari Lagi</td>
        <td style="text-align: center"><center><font><a href="?page=input_jamaah_rahmah&NomorUmroh=<?php echo $myData['kd_umroh']; ?>" target="_self" alt="Edit Data" id="kuota"  class='btn green btn-sm' title = "Booking"><i class=" pe-7s-upload"></i></a>&nbsp;
 ...
 <a onclick="window.location.href='?page=Detail-Umroh&NomorUmroh=<?php echo $myData['kd_umroh']; ?>'"  target="_self" alt="Detail umroh" id=""  class='btn green btn-sm' title="Detail"> <i class=" pe-7s-look"></i></a>&nbsp;
 <a href="modul/<?php echo $myData['itinenary']; ?>"  target="_blank" alt="Detail umroh" id=""  class='btn green btn-sm' title="Itinenary"> <i class=" pe-7s-plane"></i></a>&nbsp;</center>
    </font> </td>
         </tr>
        <?php } ?>
