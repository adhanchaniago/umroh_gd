...
<?php
session_start();
if($_SESSION['FirstName'] == 'DataJamaahBerkah') {header('Location: ?page=DataJamaahBerkah');} ;
...
WHERE packages_program='Berkah' and travel='$Travel' ";
...
if(isset($_POST['btnCari'])){
  if($_POST) {
    ...
    FROM jamaah_daftar
    LEFT JOIN track_jamaah on jamaah_daftar.nomor_id=track_jamaah.nomor_id
    WHERE  depart LIKE '%$txtKataKunci%' and packages_program='Berkah'  and travel='$Travel'
  ...
  FROM jamaah_daftar
  LEFT JOIN track_jamaah on jamaah_daftar.nomor_id=track_jamaah.nomor_id
  WHERE packages_program='Berkah'  and travel='$Travel' ORDER BY nomor_id ASC LIMIT $hal, $row";
}
    ...
    <div class="row">
            ...
            <form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="form1" target="_self" id="form1"  class="form-horizontal" role="form">
                             <select name="txtKataKunci" bgcolor="blue" class="btn blue ">
                               ...
                               <?php
    $bacaSql = "SELECT DISTINCT depart, packages_program FROM track_jamaah WHERE packages_program='Berkah'";
...
<?php
  ...
  ?>
        ...
        <a href="?page=edit_jamaah&Kode=<?php echo $Kode; ?>" target="_self" alt="Edit Data"  class='btn btn-xs btn-success ace-icon fa fa-pencil bigger-120' title="Edit"><i class="pe-7s-edit" ></i></a>&nbsp;
           <a href="?page=Document_Personal_Berkah&NomorID=<?php echo $myData['nomor_id']; ?> " target="_self" alt="Detail Data" class='btn btn-xs btn-info ace-icon fa fa-eye bigger-120' title="Document"><i class="pe-7s-folder" ></i></a>&nbsp;
...
?>
        ...
        <?php
        echo "
            <a href='?page=DataJamaahBerkah&hal=$prev'>
        ...
        <li class="">
            ...
            for ($h = 1; $h <= $max; $h++) {
                $list[$h] = $row * $h - $row;
                echo " <a href='?page=DataJamaahBerkah&hal=$list[$h]'>$h</a> ";
            }
            ?>
        </li>
        <li class="next">
                <?php
        echo "
            <a href='?page=DataJamaahBerkah&hal=$next'>
